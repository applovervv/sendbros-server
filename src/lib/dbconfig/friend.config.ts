...
export async function getShard(tableName: string, pkName: string, pkValue?: number | null, offset?: number, limit?: number): Promise<Pool[]> {
    const affectedPools: Pool[] = [];

    for(let i = 0; i < ShardingConfig.pools.length; i++) {
        const pool = ShardingConfig.pools[i];
        const shard = ShardingConfig.shards[i];
        
        // offset과 limit이 주어진 경우
        if (offset !== undefined && limit !== undefined) {
            const rangeStart = offset;
            const rangeEnd = offset + limit;
            
            // 샤드의 범위와 요청된 범위가 겹치는지 확인
            if (!(rangeEnd < shard.rule.range_min || rangeStart > shard.rule.range_max)) {
                affectedPools.push(pool);
            }
            continue;
        }
    ...
    return  affectedPools.length > 0 ? affectedPools : [defaultPool];
  }
